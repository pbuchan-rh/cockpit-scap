...
**The fix:** Both scan invocations are wrapped in `flock -n -E 99 /var/lib/cockpit-scap/.scan.lock oscap ...` (note: `flock` does not accept a `--` end-of-options separator — its syntax is `flock [options] <file> <command> [args...]` directly, no separator; this was caught by manually testing the conflict and passthrough exit codes before shipping). The lock is shared across host and container scans, so neither tab nor any session can start a second scan while one is running anywhere. `flock -n` fails fast (doesn't block waiting for the lock) and exits with the conflict code `99` if it can't acquire it, which the `.catch()` handler in both files checks for explicitly (`err.exit_status === SCAN_LOCK_CONFLICT_CODE`) to show "a scan is already running" instead of a generic error. The lock is held only as long as the wrapped process is alive, so it releases automatically on completion, error, or cancellation — no separate cleanup path needed. See REQ-207.

**Regression caught post-deploy:** the activity-log `flock` fix above (write via `cockpit.spawn(['bash', '-c', ...], { input: line })` piping the line to `cat >> log` on stdin) shipped initially with a real bug — `cockpit.spawn()`'s `input` option does not close the child's stdin, so `cat` blocked forever waiting for EOF that never arrived, holding the `flock` indefinitely. Every `appendActivityLog()` call after that hung silently (fire-and-forget, so nothing surfaced); the log simply stopped growing. Caught by manually invoking the same `cockpit.spawn()` call from a live browser session and observing it never resolve, versus the identical script running fine over plain SSH where stdin closes naturally. Fixed by passing the line as a script argument instead of stdin (`printf '%s\n' "$line" >> "$log"`), removing the stdin dependency entirely. Lesson: a shell pattern verified via SSH is not the same as the same pattern verified via `cockpit.spawn()` — the two have different stdin-handling semantics, so the actual call path must be tested directly.
...
### Retention pruning was a silent deletion

An external code audit (2026-06-19) flagged that `pruneHistoryByType()` deletes scan history directories once the configured retention limit is exceeded, but never logged the deletion anywhere — no activity log entry, no journal entry, nothing. Manual deletes (`scan_delete`) and the bulk `Clear Scan Data` action both produce an audit trail; automatic retention pruning did not, despite removing the same kind of data. For a compliance tool, "scan results disappeared and there's no record of why" is a real gap even though the deletion itself is intentional and expected behavior.

**The fix:** `pruneHistoryByType()` now appends a `scan_prune` activity log entry (scan type + count removed) whenever it deletes anything. Since `pruneHistoryByType()` is the single shared function called by both `host-scan.js` and `container-scan.js` (and from `settings.js` when retention limits change), this one change covers pruning triggered from either scan tab. See REQ-208.
